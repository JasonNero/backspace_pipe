...
def switch_sel(curr_dep, new_dep, replace=True, grouped=False):
    ...
    for shape in sel:
        ref = pmc.ls(pmc.referenceQuery(shape, referenceNode=True, topReference=True))[0].referenceFile()

        if ref.isLoaded():
            logger.debug("now processing... " + str(ref))
            ...
            if new_dep == "ASS":
                ...
                new_path = old_path.replace("_SHD_REF.ass".format(curr_dep), "_{}_REF.ma".format(new_dep))
            else:
